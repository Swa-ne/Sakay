...
export default function ReportsLayout({ children }: { children: ReactNode }) {
    ...
    const tabs = [
        ...
        { name: 'Driver Reports', value: 'DRIVER', icon: <DriverReport /> },
    ];

    const filteredReports = (type: TypesOfReport | 'all') =>
        reports.filter((report) => (type === 'all' ? true : report.type_of_report === type));
                                ...
                                <TabsContent key={tab.value} value={tab.value} className='flex-1'>
                                    <div className='space-y-3'>
                                        {filteredReports(tab.value as TypesOfReport | 'all').map((report) => (
                                            <Link
                                                href={`/reports/${report.type_of_report.toLocaleLowerCase()}/${report._id}`}
                                                key={report._id}
                                                className='flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors'
                                            >
                                                <div className='flex items-center space-x-3'>
                                                    <div className='text-gray-400'>
                                                        {report.type_of_report === 'INCIDENT' ? (
                                                            <IncidentReport />
                                                        ) : report.type_of_report === 'PERFORMANCE' ? (
                                                            <PerformanceReport />
                                                        ) : report.type_of_report === 'DRIVER' ? (
                                                            <DriverReport />
                                                        ) : null}
                                                    </div>
                                                    <div>
                                                        <p className='font-medium text-sm'>
                                                            {report.driver
                                                                ...
                                                                : `${report.bus.bus_number} - ${report.bus.plate_number}`}
                                                        </p>
                                        ...
                                        {filteredReports(tab.value as TypesOfReport | 'all').length === 0 && (
                                            <p className='text-sm text-gray-400 text-center italic'>No reports found.</p>
                                        )}
                                    </div>
                                </TabsContent>
                            ))}
