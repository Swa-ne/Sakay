...
const ManageAccount = () => {
    const { accounts, units, assignDriverToUnit, userPage, setUserPage, unitPage, setUnitPage } = useManageAccounts();
    ...
    const paginatedAccounts = filteredAccounts.slice((userPage - 1) * itemsPerPage, userPage * itemsPerPage);

    // Pagination for units
    const totalUnitPages = Math.ceil(units.length / itemsPerPage);
    const paginatedUnits = units.slice((unitPage - 1) * itemsPerPage, unitPage * itemsPerPage);
            ...
            <div className='flex justify-center mt-10 w-full'>
                <div className='flex items-center gap-1'>
                    ...
                    {pagesToShow.map((page, index) =>
                        typeof page === 'number' ? (
                            <Button key={index} variant={currentPage === page ? 'default' : 'outline'} size='sm' onClick={() => setPage(page)}>
                                {page}
                            </Button>
                        ) : (
    ...
    return (
        <div className='p-4 md:p-5 w-full min-h-screen'>
            ...
            <div className='lg:hidden mb-4'>
                <Button onClick={() => setShowSidebar(!showSidebar)} className='w-full'>
                    ...
                    <Tabs defaultValue={tabs[0].name} className='w-full'>
                        ...
                        <TabsContent value='Accounts'>
                                ...
                                <div className='flex flex-wrap gap-2'>
                                    {['all', 'COMMUTER', 'DRIVER', 'ADMIN'].map((r) => (
                                        <Button
                                            ...
                                            className={`px-3 py-2 md:px-4 md:py-3 rounded text-sm md:text-base ${filterRole === r ? 'bg-primary text-white' : 'bg-gray-200'}`}
                                        >
                            ...
                            <div className='w-full overflow-x-auto'>
                                <AccountTable filteredAccounts={paginatedAccounts} units={units} assignDriverToUnit={assignDriverToUnit} />
                            </div>
                            {renderPagination(userPage, totalAccountPages, setUserPage)}
                        </TabsContent>
                        ...
                        <TabsContent value='Units'>
                            ...
                            {renderPagination(unitPage, totalUnitPages, setUnitPage)}
                        </TabsContent>
                    </Tabs>
                </div>
            </div>
