    ...
    return (
        <div className='flex flex-col min-h-screen w-full p-3 md:p-5 space-y-2 md:space-y-3 overflow-hidden'>
            <div className='w-full flex-grow flex flex-col lg:flex-row gap-3 md:gap-5 overflow-hidden h-0'>
                {/* Announcements List - Full width on mobile, half on desktop */}
                <div 
                    ref={announcementRef} 
                    className='w-full lg:w-1/2 bg-background rounded-xl md:rounded-2xl p-3 md:p-5 overflow-y-auto h-full'
                >
                    ...
                    {announcements.map((ann) => (
                        <Link 
                            ...
                            key={ann._id} 
                            className='h-16 md:h-20 flex items-center justify-between border gap-2 rounded-md p-2 md:pl-3 md:pr-1.5 cursor-pointer mb-2'
                        >
                            <label className='w-2/3 flex flex-col justify-center hover:underline cursor-pointer'>
                                <span className='truncate w-full block text-sm md:text-base'>
                                    <b>{ann.headline}</b> - {ann.content.substring(0, 30)}{ann.content.length > 30 ? '...' : ''}
                                </span>
                ...
                <div className='w-full lg:w-1/2 bg-background rounded-xl md:rounded-2xl overflow-hidden relative h-full p-3 md:p-5'>
                    {children}
                </div>
            </div>
        </div>
