...
function StarRating({ numStars = 5, icon, setValue, value, disabled, starSize = 'size-28', showcase, iconProps = {}, wrapperProps = {} }: StarWrapperProps) {
        ...
        <div className={cn('flex items-center gap-1', wrapperClassName)} {...restWrapperProps}>
            {Array.from({ length: numStars }, (_, i) => {
                ...
                const styledIconProps: LucideProps = {
                    ...
                    className: cn(
                        `fill-primary stroke-primary ${starSize}`,
                        {
                            'opacity-90 pointer-events-none': disabled,
                ...
                return <div key={i}>{IconComponent ? <IconComponent {...styledIconProps} /> : <StarIcon {...styledIconProps} />}</div>;
            })}
        </div>
    );
}
